module MicroHs.ExpPrint(toStringCMdl, toStringCMdl2, toStringCMdl3, toStringP, encodeString, combVersion) where
...
toStringCMdl3 :: (Ident, [LDef]) -> (Int, String)
toStringCMdl3 (mainName, ds) =
  let
    dMap = M.fromList ds
    -- Shake the tree bottom-up, serializing nodes as we see them.
    -- This is much faster than (say) computing the sccs and walking that.
    ...
    dfs n = do
      ...
      case M.lookup n seen of
        ...
        Nothing -> do
          ...
          (i', seen', r') <- get
          put (i'+1, M.insert n (ref i') seen', def r' (i', e))
    (_,(ndefs, defs, res)) = runState (dfs mainName) (0, M.empty, toStringWasm emain)
    ref i = Var $ mkIdent $ "_" ++ show i
    findIdentIn n m = fromMaybe (errorMessage (getSLoc n) $ "No definition found for: " ++ showIdent n) $
                      M.lookup n m
    findIdent n = findIdentIn n defs
    ...
    def :: (String -> String) -> (Int, Exp) -> (String -> String)
    def r (i, e) =
      -- TODO: ":" + show i becomes table.set
      (iComb ++) . toStringWasm (substv e) . ((appNode ++ fDef ++ "\n") ++) . r 
        where
          iComb   = emit [prim "I"]
          appNode = emit [ StructNew AppNodeType ]
          fDef    = emit ( [ LocalSet TempVar, 
                             I32Const i, TableGet TableFnDefs,
                             LocalGet TempVar ] ++
                             nodeSet RightF )
          -- function definition leads to a table reference change
  in
    case dupInstances ds of
      (n1 : n2 : _) : _ -> errorMessage (getSLoc n1) $ "Duplicate instance " ++ unmangleInst (showIdent n1) ++ " at " ++ showSLoc (getSLoc n2)
      _ -> (ndefs, tInit ++ res "")
        where
          tInit = emit [ TableDecl TableFnDefs ndefs AppNodeType ,
                         TableFill ndefs ]
