...
generateWats :: IO ()
generateWats = mapM_ (main2 . (filePrefix ++)) (testFiles \\ noWork)

filePrefix :: String
filePrefix = "/home/spirit/Desktop/Programs/Thesis/MicroHs-ghc/tests/"

noWork :: [String]
noWork = ["PatSyn.hs", "TestMap.hs", "Readline.hs", "RecMdlA.hs", "RecMdl.hs"
         ,"IOTest.hs", "Cpp.hs", "PatSynE.hs"]

testFiles :: [String]
testFiles =
  ["Arith.hs", "Delay.hs", "FunDep.hs", "LocalPoly.hs", "PatSyn.hs","TestMap.hs",
  "Array.hs", "Deriving.hs", "Guard.hs", "Mask.hs", "PolyKind.hs", "Text.hs",
  "Bang.hs", "DfltSig.hs", "Hello.hs", "Misc.hs", "Rank2.hs", "Throw.hs",
  "BindPat.hs", "Dict.hs", "HigherKind.hs", "MString.hs", "Read.hs", "ThrSt.hs",
  "BitCount.hs", "Do.hs", "ImpMet.hs", "Multi.hs", "Readline.hs", "Typeable.hs",
  "Block.hs", "EmptyCase.hs", "Infer.hs", "MultiIf.hs", "RecMdlA.hs", "TypeApp.hs",
  "Bounded.hs", "Enum.hs", "Infix.hs", "MutRec.hs", "RecMdl.hs", "TypeEq.hs",
  "Bytestring.hs", "Eq1.hs", "Info.hs", "MVar.hs", "Record.hs", "TypeLits.hs",
  "ByteStringIO.hs", "Eq.hs", "Integer.hs", "Newtype.hs", "RtsExn.hs", "TypeOp.hs",
  "Case.hs", "Exists.hs", "IOArray.hs", "Nfib.hs", "Serdes.hs", "Unicode.hs",
  "Catch.hs", "Fac.hs", "IOTest.hs", "NoForall.hs", "Sieve.hs", "Via.hs",
  "Class.hs", "FArith.hs", "Irref.hs", "NumUnder.hs", "Stable.hs", "View.hs",
  "Coerce.hs", "Floating.hs", "LameCase.hs", "OrPat.hs", "StandDer.hs", "Word.hs",
  "Concur.hs", "Forall.hs", "ListCompr.hs", "ParseInd.hs", "ST.hs",
  "Cpp.hs", "ForeignC.hs", "ListTest.hs", "PartRed.hs", "Storable.hs",
  "CString.hs", "ForeignPtr.hs", "LitMatch.hs", "PatBind.hs", "StringTest.hs",
  "Default.hs", "Fork.hs", "LocalFix.hs", "PatSynE.hs", "Symbol.hs"]

fileSuffix :: String
fileSuffix = "/home/spirit/Desktop/Programs/Thesis/test-results/"

outputPath :: FilePath -> FilePath
outputPath fp = 
  let
    dropped = drop (length filePrefix) fp
    changed = take (length dropped - 3) dropped ++ "-wasmgc.wat"
  in
    fileSuffix ++ changed

main2 :: FilePath -> IO ()
main2 fp = do
  let args = [fp, "-o", outputPath fp]
  dir <- getMhsDir
  dataDir <- getDataDir
  case args of
    ["-h"] -> putStrLn usage
...
mainCompile flags mn = do
  ...
  dumpIf flags Dwasmgc $
    mapM_ (\ (i, e) -> putStrLn $ showIdent i ++ " = " ++ toStringWasm e "") allDefs
...
--    putStrLn "done"
    ...
    if outFile `hasTheExtension` ".comb" then
     ...
     else if outFile `hasTheExtension` ".comb3" then
      writeFile outFile outData3
     else if outFile `hasTheExtension` ".wat" then
      writeBetween outFile "/home/spirit/Desktop/Programs/Thesis/MicroHs-ghc/SKeleton.wat" outData3
     else do
...
mainCompileC flags ppkgs infile = do
  ...
  TTarget _ compiler ccflags _ conf <- readTarget flags dir
  extra <- fromMaybe "" <$> lookupEnv "MHSEXTRACCFLAGS"
  let dcc = compiler ++ " -w -Wall -O0 -g -I" ++ dir ++ "/src/runtime " ++
                        ccflags ++ " " ++
                        incs ++ " " ++
                        defs ++ " " ++
                        extra ++ " " ++
...
-- TODO: Refactor this and WasmGC.MyLib and take out common where clause
writeBetween :: FilePath -> FilePath -> String -> IO ()
writeBetween outFile rtsFile code = do
  fb <- BL.readFile rtsFile
  let ft                             = TE.decodeUtf8 fb
      (codeTableDecl, codeProgGraph) = T.breakOn "\n" (T.pack code)
      td                             = "Table Declaration"
      pg                             = "Program Graph"
      untilStartTable                = f1 (start td) ft
      restTable                      = f2 (end td) ft

      r1 = T.concat [ untilStartTable, T.pack (start td),
                      codeTableDecl, "\n",
                      T.pack (end td), restTable ]

      untilStartProg                 = f1 (start pg) r1
      restProg                       = f2 (end pg) r1

      r2 = T.concat [ untilStartProg, T.pack (start pg),
                      codeProgGraph, "\n",
                      T.pack (end pg), restProg ]
  TIO.writeFile outFile r2
    where
      start x  = ";; " ++ x ++ " Start\n"
      end   x  = ";; " ++ x ++ " End\n"
